  ...
  const handleSubmit = (e) => {
    ...
    if (!userData.user_password) {
      ...
      formErrors.user_password =
        'Password must contain alphabetical letters, numbers and atleast one special character';
    }
  ...
  useEffect(() => {
    ...
    if (isDataSaved) {
      const timer = setTimeout(() => {
        navigate('/'); // Change the path to your home page
      }, 5000);

      // Clear the timer if the component unmounts before the 5 seconds
      return () => {
        clearTimeout(timer);
        // Trigger the navigation when the component unmounts
        navigate('/');
      };
    }
      ...
      <FormContainer>
        <Form onSubmit={handleSubmit}>
          ...
          <FormRow>
            {isDataSaved && (
              <SuccessMessage>
                User data stored successfully! Proceed to <Link to="/">login</Link>.
              </SuccessMessage>
            )}
            {!isDataSaved && submitError && (
