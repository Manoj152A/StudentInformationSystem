    ...
    <div style={containerStyle}>
      <main style={{ color: '#8B0000' }}>
        ...
        <div style={{ display: 'flex', marginLeft: '280px' }}>
          ...
          >
            ...
            {institutionNames.map((institution) => (
              <option value={institution.name} key={institution.id}>
                {institution.name}
              </option>
            ))}
          </select>
        ...
        <div style={{ display: 'flex', flexDirection: 'row' }}>
          <table style={{ width: '80%', border: '0.8px solid darkmagenta', borderCollapse: 'collapse',marginLeft: '120px',boxShadow: '4px 4px 8px rgba(0, 0, 0, 0.2)' }}>
            <thead>
              <tr>
              ...
              {searchResults.map((student, index) => (
                <tr key={index}>
                  ...
                  <td style={{ padding: '8px', color: 'darkmagenta', border: '1px solid darkmagenta', position: 'relative' }}>
                    {student.username}<button style={{ position: 'relative', left:'220px',border: '100px', backgroundColor: '#34ff3e', color: '#373e37', borderRadius: '3px',boxShadow: '4px 4px 8px rgba(0, 0, 0, 0.2)' }}
                     onClick={handleSearch}
                     onMouseEnter={(e) => {
                       e.target.style.transform = 'scale(1.1)';
                     }}
